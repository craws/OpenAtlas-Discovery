...
console.log(
	"Content repo commit hash: ",
	childProcess.execSync(`git -C ${clonedContentPath} rev-parse HEAD`).toString().trim(),
);
...
function cloneRepo(targetpath, repo, branch, useHttp = true) {
	...
	if (useHttp) {
		console.log(
			`Attempting to clone ${getBranch(branch)} on https://github.com/${repo}.git to ${targetpath}`,
		);
		childProcess.execSync(
			`git clone ${getBranch(branch)} https://github.com/${repo}.git ${targetpath}`,
		);
	} else {
		console.log(
			`Attempting to clone ${getBranch(branch)} on git@github.com:${repo}.git to ${targetpath}`,
		);
		childProcess.execSync(
			`git clone ${getBranch(branch)} git@github.com:${repo}.git ${targetpath}`,
		);
	}
