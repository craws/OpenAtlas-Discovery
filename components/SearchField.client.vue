...
import { type Query, type Search } from "~~/types/query";
          ...
          <template #item="{ item }">
            <v-list-item v-slot="{ active }" @click="filterOperator = item">
              <span class="text-capitalize">
                {{ $t(`global.entity.${item.title}`) }}</span
              >
            </v-list-item>
          </template>
          <template #selection="{ item }" class="text-capitalize">
            <span class="text-capitalize">{{
              $t(`global.entity.${item.title}`)
            }}</span>
          </template>
        </v-select>
      </v-col>
...
.search-field {
  .v-field {
    ...
    box-shadow: -2px 3px 1px -2px var(--v-shadow-key-umbra-opacity, rgba(0, 0, 0, 0.2)),
