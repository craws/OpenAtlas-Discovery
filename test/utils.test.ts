...
import { useFormatDateTime, useSafeRead } from "../composables/utils";

describe("useSafeReadTest", () => {
  const object = {
    name: "Andreas",
    "@id": 1,
    properties: {
  ...
  test("should return correct values", () => {
    ...
    expect(useSafeRead(object, 'properties["some_property"]')).toBe(
      "something"
    );
    expect(useSafeRead(object, "properties['some_property']")).toBe(
      "something"
...
describe("useFormatDateTime test", () => {
  test("should return correct values", () => {
